/* IRhard.h header version 1.1.0 by u/inflannel, 5 September 2018. <-- Check the date.
 *       By the time you see this, you may be looking for a different IRhard.h.
 *
 ...
 * Usage:
 *   #include IRhard.h
 *   IR_init(pin [,rcid] [,validate]); // hooks the hardware interrupt to the specified pin
 ...
 *   //   Alternately: you can set IR_RCID to a specific remote control ID at any time.
...
bool IR_No_Validate = true, IR_loaded = false;
      ...
      if (wl > 2500) { acc = 0; abit = 1; } // 110% of bit-1 wavelength is "too long", causing reset
        ...
        if (abit < 0x80000000) { abit <<= 1; } // if not 32 bits, adjust to keep reading
          ...
          if ((IR_RCID == 0) || ((acc & 0xFFFF) == IR_RCID)) { // correct remote.
            // If validating, make sure key byte matches the inverse byte (ie 0xED12: 0xED + 0x12 == 0xFF)
...
void IR_init(uint8_t pin, unsigned int id = 0, bool b = false) {
  ...
  if (IR_loaded || (IR_int_num < 0)) { // can't attach interrupt to specified pin
  ...
  else { // A hardware interrupt pin is passed, as is the normal (working) case.
    IR_loaded = true;
    ...
    attachInterrupt(IR_int_num, _IR_interrupt, FALLING); // hook into hardware interrupt
